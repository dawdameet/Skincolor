...
def extract_skin_tone(image: np.ndarray, exclude_beard: bool = False) -> np.ndarray:
    ...
    create a smoothed convex hull mask for the face region, optionally remove the lower
    portion (heuristically excluding beard), and compute the average skin tone.
    """
    h, w, _ = image.shape
    ...
    with mp_face_mesh.FaceMesh(
        ...
        # Create a binary mask from the convex hull of the landmarks
        mask = np.zeros((h, w), dtype=np.uint8)
        points_np = np.array(points, dtype=np.int32)
        forehead_hull = cv2.convexHull(points_np[:7])
        cheek_hull = cv2.convexHull(points_np[7:21])
        nose_hull = cv2.convexHull(points_np[21:])

        cv2.fillConvexPoly(mask, points_np, 255)
        cv2.fillConvexPoly(mask, forehead_hull, 255)
        cv2.fillConvexPoly(mask, cheek_hull, 255)
        ...
        # # If excluding beard, remove bottom portion of the mask.
        # if exclude_beard:
        #     # Compute bounding rectangle of the face
        #     x, y, w_rect, h_rect = cv2.boundingRect(points_np)
        #     # Exclude lower 30% of the bounding box
        #     exclusion_y = y + int(h_rect * 0.7)
        ...
        # # Extract skin region using the smoothed mask
        ...
        # # Compute average color (RGB) from the skin region
...
def main():
    parser = argparse.ArgumentParser(
        description="Extract overall average skin tone from one or more images, optionally excluding beard regions."
    )
